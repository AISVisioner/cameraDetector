...
func (server *Server) setupRouter() {
	...
	router.POST("/admins", server.createAdmin)
	router.POST("/admins/login", server.loginAdmin)
	router.POST("/tokens/renew_access", server.renewAccessToken)

	authRoutes := router.Group("/").Use(authMiddleware(server.tokenMaker))
	authRoutes.POST("/visitors", server.createVisitor)
	authRoutes.GET("/visitors/:id", server.getVisitor)
	authRoutes.GET("/visitors", server.listVisitors)
